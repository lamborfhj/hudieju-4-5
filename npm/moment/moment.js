...
(function (global, factory) {
  ...
  function configFromISO(config) {
    var i,
        l,
        string = config._i,
        match = /^\s*((?:[+-]\d{6}|\d{4})-(?:\d\d-\d\d|W\d\d-\d|W\d\d|\d\d\d|\d\d))(?:(T| )(\d\d(?::\d\d(?::\d\d(?:[.,]\d+)?)?)?)([+-]\d\d(?::?\d\d)?|\s*Z)?)?$/.exec(string) || /^\s*((?:[+-]\d{6}|\d{4})(?:\d\d\d\d|W\d\d\d|W\d\d|\d\d\d|\d\d|))(?:(T| )(\d\d(?:\d\d(?:\d\d(?:[.,]\d+)?)?)?)([+-]\d\d(?::?\d\d)?|\s*Z)?)?$/.exec(string),
        allowTime,
        dateFormat,
        timeFormat,
        tzFormat;
  ...
  function configFromRFC2822(config) {
    var match = /^(?:(Mon|Tue|Wed|Thu|Fri|Sat|Sun),?\s)?(\d{1,2})\s(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s(\d{2,4})\s(\d\d):(\d\d)(?::(\d\d))?\s(?:(UT|GMT|[ECMP][SD]T)|([Zz])|([+-]\d{4}))$/.exec(preprocessRFC2822(config._i)),
        parsedArray;
  ...
  function configFromString(config) {
    var matched = /^\/?Date\((-?\d+)/i.exec(config._i);
  ...
  function createDuration(input, key) {
    ...
    } else if (match = /^(-|\+)?(?:(\d*)[. ])?(\d+):(\d+)(?::(\d+)(\.\d*)?)?$/.exec(input)) {
      sign = match[1] === '-' ? -1 : 1;
      duration = {
        y: 0,
    ...
    } else if (match = /^(-|\+)?P(?:([-+]?[0-9,.]*)Y)?(?:([-+]?[0-9,.]*)M)?(?:([-+]?[0-9,.]*)W)?(?:([-+]?[0-9,.]*)D)?(?:T(?:([-+]?[0-9,.]*)H)?(?:([-+]?[0-9,.]*)M)?(?:([-+]?[0-9,.]*)S)?)?$/.exec(input)) {
      sign = match[1] === '-' ? -1 : 1;
      duration = {
  ...
  hooks.defaultFormatUtc = 'YYYY-MM-DDTHH:mm:ss[Z]'; // If passed a locale key, it will set the locale for this
  ...
  proto$2.humanize = function (argWithSuffix, argThresholds) {
    if (!this.isValid()) {
      return this.localeData().invalidDate();
    }
